  ...
  return (
    <nav className="bg-white dark:bg-gray-800 shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <a href="/" onClick={handleLogoClick}
           ...
           dark:hover:text-indigo-400 transition-colors cursor-pointer">Mohammed Maibu </a>
          ...
          <div className="hidden md:flex space-x-6">

            {
              ...
              navItems.map((item) => (
                ...
                <button key={item.name} onClick={() => handleNavClick(item.path, item.name)}
                  className="text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 
              transition bg-transparent border-none cursor-pointer"> {item.name} </button>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button onClick={toggleMenu}
              ...
              focus:ring-2 focus:ring-indigo-500 rounded-md" aria-label="Toggle Menu"
      ...
      ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`} >
        ...
        <div className="bg-white dark:bg-gray-800 px-4 pt-2 pb-4 space-y-2 shadow-md border-t 
...

          {
            ...
            navItems.map((item) => (
              ...
              <button key={item.name} onClick={() => handleNavClick(item.path, item.name)}
                ...
                className="block w-full text-left text-gray-800 dark:text-gray-200 hover:bg-indigo-100 
      dark:hover:bg-indigo-500 hover:text-black dark:hover:text-white px-3 py-2 rounded 
      transition-all duration-200 bg-transparent border-none cursor-pointer" > {item.name} </button>
            ))
    ...
    1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
              ...
              <button onClick={() => setShowPopup(false)} className="ml-4 text-white hover:text-gray-200 transition-colors">
